    ...
    <div class="w-full min-h-screen">
        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl lg:text-4xl font-semibold tracking-tight text-foreground">
              <span class="text-primary"><%= @client.user.name %></span>'s Progress Photos
            </h1>
            <p class="mt-2 text-dim text-base lg:text-lg">Visual progress tracking</p>
          </div>
          <.link
      ...
      <div class="w-full px-0 sm:px-2 lg:px-4 py-6">
        <%= if length(@photos) == 0 do %>
          <div class="bg-card rounded-2xl shadow-sm border border-line p-12 text-center">
            ...
            <p class="mt-2 text-sm text-dim">This client hasn't uploaded any progress photos.</p>
          </div>
        <% else %>
            ...
            <%= for photo <- @photos do %>
              <div class="bg-card rounded-xl shadow-sm border border-line overflow-hidden">
                ...
                <div class="p-4">
                  ...
                  <%= if photo.notes do %>
                    <p class="text-sm text-dim mt-1 line-clamp-2"><%= photo.notes %></p>
                  <% end %>
                </div>
              </div>
            <% end %>
