    ...
    def render(assigns) do
      ...
      <div class="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto">
          ...
          <div class="bg-card rounded-2xl shadow-xl p-8 mb-6 border border-purple-100">
            <div class="flex items-center justify-between">
              <div>
                <h1 class="text-4xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent mb-2">
                  Client Notes
                </h1>
                <p class="text-dim">Keep track of important client observations and progress notes</p>
              </div>
              <button
                phx-click="openModal"
                class="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-foreground font-semibold px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 flex items-center gap-2">
          ...
          <%= if @showModal do %>
            <div class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
              <div class="bg-card rounded-2xl shadow-2xl w-full max-w-2xl transform transition-all">
                <div class="p-6 border-b border-line bg-gradient-to-r from-purple-50 to-pink-50">
                  <h2 class="text-2xl font-bold text-foreground">Add New Note</h2>
                  <p class="text-dim text-sm mt-1">Document important client information and observations</p>
                </div>
                ...
                <.form phx-submit="addNote" for={@newForm}>
                  <div class="p-6 space-y-5">
                    <div>
                      ...
                      <.input
                        ...
                        required
                        field={@newForm[:date]}
                        class="w-full px-4 py-3 rounded-lg border border-line focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                      />
                    </div>

                    <div>
                      <label class="block text-sm font-semibold text-foreground mb-2">Note</label>
                      <.input
                        type="text"
                        field={@newForm[:notes]}
                        class="w-full px-4 py-3 rounded-lg border border-line focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                  ...
                  <div class="p-6 bg-card rounded-b-2xl flex gap-3">
                    <button
                      type="button"
                      phx-click="openModal"
                    ...
                    <.button class="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-foreground font-semibold px-6 py-3 rounded-xl shadow-md hover:shadow-lg transition-all duration-200">
                      Save Note
                    </.button>
                  </div>
                </.form>
          ...
          <%= if length(@notes) > 0 do %>
            <div class="grid gap-4">
              <%= for note <- @notes do %>
                <div class="bg-card rounded-xl shadow-md hover:shadow-lg transition-all duration-200 border border-purple-100 overflow-hidden">
                  <div class="p-6">
                    <div class="flex items-start justify-between gap-4">
                      <div class="flex-1">
                        ...
                        <div class="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-4 border border-purple-100">
                          <p class="text-foreground leading-relaxed"><%= note.notes %></p>
                        </div>
                      </div>
                      <button
                        phx-click="deleteNote"
                        phx-value-id={note.id}
                        class="flex-shrink-0 bg-danger/10 hover:bg-danger/10 text-danger font-semibold px-4 py-2 rounded-lg transition-all duration-200 border border-danger flex items-center gap-2">
          ...
          <% else %>
            <div class="bg-card rounded-2xl shadow-lg p-12 text-center border border-purple-100">
              ...
              <h3 class="text-2xl font-bold text-foreground mb-2">No Notes Yet</h3>
              <p class="text-dim mb-6">Start documenting important client information by adding your first note.</p>
              <button
                phx-click="openModal"
                class="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-foreground font-semibold px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                </svg>
                Add Your First Note
